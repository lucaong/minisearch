...
export const behavesLikeSearchableMap = function (name, SearchableMap) {
  describe(name, () => {
    ...
    describe('clear', () => {
      it('empties the tree', () => {
        const tree = SearchableMap.from(keyValues)
        tree.clear()
    ...
    describe('get', () => {
      it('gets the value at key', () => {
        const key = 'foo'
        const value = 42
        const tree = SearchableMap.fromObject({ [key]: value })
    ...
    describe('has', () => {
      ...
      it('returns false if the given key does not exist in the tree', () => {
        const tree = SearchableMap.fromObject({ something: 42 })
    ...
    describe('set', () => {
      ...
      it('overrides a value at key if it already exists', () => {
        const tree = SearchableMap.fromObject({ foo: 123 })
        const key = 'foo'
        const value = 42
        tree.set(key, value)
        expect(tree.get(key)).toBe(value)
    ...
    describe('values', () => {
      it('returns an iterator of values', () => {
        const tree = SearchableMap.fromObject(object)
